...
function checkLocalUrl(value, from) {
  ...
  // Przykłady NB odsyłają do pełnych poradników na obecnej witrynie Docsify.
  if (profile.prototype && /^https?:\/\//.test(value) && url.hash.startsWith('#/')) return;
...
const hostname = new URL(profile.url).hostname;
if (files.has('CNAME')) {
  if ((await fs.readFile(path.join(buildDir, 'CNAME'), 'utf8')).trim() !== hostname) {
    errors.push('CNAME i domena w profilu nie są zgodne.');
  }
} else if (!['localhost', '127.0.0.1'].includes(hostname)) {
  errors.push('Brak CNAME dla domeny publikacji.');
}
